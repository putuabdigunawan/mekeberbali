import { c as createAstro, a as createComponent, m as maybeRenderHead, d as renderComponent, b as addAttribute, r as renderTemplate } from './astro/server_FPTJsa35.mjs';
import 'kleur/colors';
import { h as humanize, s as slugify, p as plainify, c as config } from './ClientRouter_Ba-Ivvbk.mjs';
import { d as dateFormat, r as readingTime } from './readingTime_B27unFHI.mjs';
...
const $$Blog = createComponent(($$result, $$props, $$slots) => {
  ...
  return renderTemplate`${maybeRenderHead()}<div class="row"> ${posts.map((item) => renderTemplate`<div class="mb-8 md:col-6 lg:col-4"> <div class="card"> ${renderComponent($$result, "Image", $$Image, { "class": "card-img aspect-video", "width": 335, "height": 210, "src": item.data.image, "alt": "" })} <div class="card-content"> <div class="card-tags space-x-1"> ${item.data.categories.map((category) => renderTemplate`<a class="tag"${addAttribute(`/categories/${slugify(category)}`, "href")}> ${humanize(category)} </a>`)} </div> <h3 class="h4 card-title"> <a${addAttribute(`/blog/${item.id}`, "href")}>${item.data.title}</a> </h3> ${item.body && renderTemplate`<p class="mb-4 text-text"> ${plainify(item.body?.slice(0, Number(summary_length)))}...
...
}, "/Users/user/Documents/all_project/astroboy/pinwheel-astro-main/src/layouts/components/Blog.astro", void 0);
